...
###### 3、为什么如果连接是从服务器到客户端，所有的读写都应采用非阻塞式IO，阻塞式读写是提供给客户端使用的：

答：这是因为服务器需要同时处理多个客户端的请求，如果采用阻塞式读写操作，服务器在等待一个客户端的读写操作完成时，会被阻塞，无法处理其他客户端的请求。而非阻塞式读写操作可以立即返回，这样服务器就可以处理其他客户端的请求，提高了CPU的利用率。另外，由于客户端数量较少，采用阻塞式读写操作不会对CPU的利用率造成太大影响。
但是，客户端也可以采用非阻塞式读写操作，这样可以提高CPU的利用率，但是可能会导致读写的不完整或者不正确。如果客户端数量较少，采用阻塞式读写操作不会对CPU的利用率造成太大影响。

------

#### day12

本次的修改中我们添加了MAC_OS的版本，我们现在的网络库不仅支持Linux还支持MAC，主要用的技术是修改Epoll类为Poller类，在Poller类中用宏定义分割两种不同版本的操作系统，因为CMake是支持跨平台的所以可以支持MAC版本。
